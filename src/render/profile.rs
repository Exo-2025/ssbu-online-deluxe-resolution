...
impl RenderProfile {
    ...
    pub fn from_settings(rps: &RenderProfileSettings) -> Self {
        let preset = match (
            ...
            (BufferMode::Triple, IndexMode::TwoBehind, true, false) => {
                RenderProfilePreset::Vanilla
            }

            (BufferMode::Double, IndexMode::OneBehind, false, true) => {
                RenderProfilePreset::LessLag
            }
...
impl<'de> Deserialize<'de> for RenderProfile {
        ...
        D: Deserializer<'de>,
    {
        ...
        let preset =
            RenderProfilePreset::deserialize(render_profile_str.into_deserializer())?;
...
pub struct RenderProfileSettings {
    ...
    // This remains part of the profile structure, but the actual
    // application function below forcibly uses 1920x1080.
...
impl RenderProfileSettings {
    pub const fn vanilla() -> Self {
        RenderProfileSettings {
            ...
            // Never allow dynamic resolution to lower the resolution.
    ...
    pub fn less_lag_ultra() -> Self {
        RenderProfileSettings {
            ...
            // Previously this used 1024x576 on real hardware.
            // It is now always 1920x1080.
            default_resolution_level: ResolutionLevel::Res1920x1080,

            // Prevent the game from dynamically lowering resolution.
            dynamic_res_enabled: false,
    ...
    pub fn less_lag_doubles() -> Self {
        RenderProfileSettings {
            ...
            // Prevent dynamic resolution from lowering it.
            dynamic_res_enabled: false,
    ...
    pub fn from_render_profile(rp: RenderProfile) -> Self {
        let mut rps = match rp.preset {
            ...
            RenderProfilePreset::LessLag => {
                RenderProfileSettings::less_lag()
            }

            RenderProfilePreset::LessLagUltra => {
                RenderProfileSettings::less_lag_ultra()
            }

            RenderProfilePreset::LessLagDoubles => {
                RenderProfileSettings::less_lag_doubles()
            }

            _ => panic!("Must specify a valid render profile"),
        };
    ...
    pub fn from_env() -> Self {
        RenderProfileSettings {
            ...
            // Read the current value for profile detection, but the
            // actual application path below will still force 1920x1080.
            default_resolution_level: sync_guest::default_game_resolution_level()
                .flatten()
                .unwrap_or(ResolutionLevel::Res1920x1080),

            // Read this for the current environment, but it is NOT
            // allowed to enable dynamic resolution when applying a profile.
            dynamic_res_enabled: sync_guest::dynamic_resolution_enabled()
                .unwrap_or(false),

            vsync_enabled: sync_guest::vsync_enabled()
                .unwrap_or(true),

            render_opts_enabled: sync_guest::render_opts_enabled()
                .unwrap_or(false),

            boost_enabled: sync_guest::fps_boost_enabled()
                .unwrap_or(false),
        }
    }
    ...
    pub fn from_bits(bits: u16) -> Option<Self> {
        Some(Self {
            ...
            index_mode: IndexMode::from_u32(
                ((bits >> 2) & 0b11) as u32
            )?,

            default_resolution_level: ResolutionLevel::from_u32(
                ((bits >> 4) & 0x7f) as u32
            )?,
...
impl RenderProfileManager {
    const fn new() -> Self {
        RenderProfileManager {
            ...
            selected_profile_settings: AtomicU16::new(
    ...
    pub fn instance() -> &'static Self {
        static RENDER_PROFILE_MANAGER: RenderProfileManager =
            RenderProfileManager::new();

        &RENDER_PROFILE_MANAGER
    }

    pub fn selected_render_profile_settings(
        &self,
    ) -> RenderProfileSettings {
    ...
    pub fn selected_render_profile(&self) -> RenderProfile {
        RenderProfile::from_settings(
            &self.selected_render_profile_settings(),
        )
    }

    pub fn active_render_profile() -> RenderProfile {
        RenderProfile::from_settings(
            &Self::active_render_profile_settings(),
        )
    }

    pub fn selected_render_profile_set_boost_enabled(
        &self,
        boost_enabled: bool,
    ) {
    ...
    pub fn apply_selected_profile_settings(&self) {
        Self::apply_render_profile_settings_immediate(
            &self.selected_render_profile_settings(),
        );
    }
    ...
    pub fn select_next_render_profile(&self) -> RenderProfile {
        ...
        let next_rp = RenderProfile::new(
            next_preset,
            selected_rp.boost_enabled(),
        );

        let next_rps =
            RenderProfileSettings::from_render_profile(next_rp);
    ...
    pub fn select_prev_render_profile(&self) -> RenderProfile {
        ...
        let prev_rp = RenderProfile::new(
            prev_preset,
            selected_rp.boost_enabled(),
        );

        let prev_rps =
            RenderProfileSettings::from_render_profile(prev_rp);
    ...
    pub fn set_auto_mode(&self, auto_mode: bool) {
        ...
        let num_connected_stations =
            StationConnectionManager::num_connected_stations();

        self.auto_select_profile(
            is_valid_online_mode(),
            num_connected_stations > 1,
        );
    }
    ...
    pub fn poll(
        ...
        let boost_buttons =
            input_snapshot.check_buttons_pressed_with_cooldown(&[
                ninput::Buttons::L
                    | ninput::Buttons::R
                    | ninput::Buttons::ZL
                    | ninput::Buttons::X,
        ...
        if !boost_buttons.is_empty() {
            ...
            self.selected_render_profile_set_boost_enabled(
                !rps.boost_enabled,
            );
        ...
        if cycle_buttons == prev {
            if self.is_auto_mode() {
                ...
                if self.selected_render_profile().preset
                    == RenderProfilePreset::Vanilla
                {
    ...
    pub fn recommended_render_profile(
        &self,
        is_online: bool,
        is_doubles: bool,
    ) -> RenderProfile {
        ...
        // If boost status is unspecified, inherit from previous
        // render profile.
        if rp.fps_boost_status == FPSBoostStatus::Unspecified {
            rp.fps_boost_status =
                match self.selected_render_profile().boost_enabled() {
    ...
    pub fn auto_select_profile(
        ...
        let rp = self.recommended_render_profile(
            is_online,
            is_doubles,
        );
    ...
    fn select_render_profile_immediate(
        &self,
        rp: RenderProfile,
    ) {
        self.selected_profile_settings.store(
            RenderProfileSettings::from_render_profile(rp)
                .to_bits(),
            Ordering::SeqCst,
        );
    }

    fn apply_render_profile_settings_immediate(
        rps: &RenderProfileSettings,
    ) {
        ...
        // ============================================================
        //
        // Do NOT use rps.default_resolution_level here.
        //
        // This deliberately overrides whatever resolution is contained
        // in the selected profile or configuration.
        //
        // This means LessLagUltra can no longer select 1024x576,
        // and a config file cannot cause this code to apply a lower
        // resolution.
        //
        let _ = sync_guest::set_default_game_resolution_level(
            ResolutionLevel::Res1920x1080,
        );

        // Never allow dynamic resolution to lower the internal
        // resolution.
        let _ =
            sync_guest::set_dynamic_resolution_enabled(false);
        ...
        if let Some(env_flags) = sync_guest::env_flags() {
            ...
            let _ =
                sync_guest::replace_env_flags(flags);
        }
        ...
        if rc.overclocker && !is_emulator {
            let oc_profile = match (
                RenderProfile::from_settings(&rps).preset,
                is_in_game(),
            ) {
            ...
            let _ =
                sync_guest::profile::apply_docked_profile(oc_profile);
        }
    }
}
...
pub(crate) fn match_init() {
    ...
    let in_real_online_match =
        is_connected && match_status != MatchStatus::Training;

    if in_real_online_match {
        if is_valid_online_mode {
            // Apply selected profile if it is a valid online match.
            RenderProfileManager::instance().auto_select_profile(
                true,
                match_status == MatchStatus::Doubles,
            );

            RenderProfileManager::instance()
                .apply_selected_profile_settings();
        } else {
            // ssbusync already enforces this, but leaving this here
            // for clarity.
